...
| `testAssertion`         | Medium   | Tests without expect() or assert calls                                   | Without an assertion the test passes for any output, so green CI hides the regression       | Playwright, custom test frameworks                     |
...
| `naming`                | Mixed    | Naming conventions (hooks, components, types)                            | Inconsistent names defeat code search and the pattern recognition that picks out hooks / components | Different naming conventions in team/project           |
