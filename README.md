...
- **Disabling `security` does not stop the postMessage wildcard check**: it fires under the `security` rule_id, but the check itself lives in `astSecurity`'s `analysis::ast_security::postmessage::check_post_message_wildcard`, which runs independently of the `security` toggle. An override that disables only `security` for a glob leaves the check firing as long as `astSecurity` stays on for those files; both toggles have to be off to stop it.
