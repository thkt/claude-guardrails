...
| `naming` | Mixed | Naming conventions. Medium for components; Low for interfaces and types, so this rule never blocks under the default threshold | Inconsistent PascalCase defeats code search and the pattern recognition that picks these out | Different naming conventions in team/project |
...
| `ast-checker-internal-failure` | High | AST checker subprocess failed before completing every structural rule | An incomplete checker run cannot establish that the edit is safe; blocking prevents an internal failure from silently bypassing structural rules |
...
- An entry whose `files` pattern fails to compile as a glob (e.g. an unbalanced `[`) is dropped in its entirety. The rest of the config, including other `overrides` entries and the top-level `rules`, is unaffected. guardrails adds a note naming the pattern it could not compile, e.g. `override entry dropped: glob pattern "src/[invalid" failed to compile in .guardrails.json`. This note is surfaced in the JSON envelope's `notes` and on stderr on every config load, regardless of whether the file being checked matches that entry's `files`.
- **Toggle granularity, not rule granularity**: an override's `rules` keys are the same coarse toggles as the top-level config, not individual `rule_id`s. Disabling `security` or `astSecurity` for a glob disables every `rule_id` that toggle covers for the matched files. For `astSecurity` that is 14 of the 15 rows listed under `AST Security Rules` above, not a single one — there is no per-path way to turn off one sub-rule while keeping its siblings on.
- **`excessive-nesting` is the one sub-rule an override cannot turn off**: it is a byte scan that runs before the parse whenever any AST rule is on, because gating it on `astSecurity` would let a config with `astSecurity` off and `eval` on reach the parser and crash it. An override that disables `astSecurity` still blocks a file nested past the safe depth.
- **Disabling `security` does not stop the postMessage wildcard check**: it fires under the `security` rule_id, but the check lives in `astSecurity`'s `analysis::ast_security::postmessage::check_post_message_wildcard`. Turning `astSecurity` off stops that check on its own. The `security` rule_id keeps firing from the string-timer and sensitive-storage patterns, so silencing the rule_id for a path takes both toggles off. The note for `security` counts 1 rule_id for the same reason: the toggle stops `dangerous-inner-html` alone.
- When an override disables a rule for the file being checked, guardrails adds a note naming the disabled rule(s) and the matching pattern(s), e.g. `override disabled rule(s) [testAssertion] for pattern(s) [src/**/*.test.ts] (testAssertion stops 1 rule_id(s))`, surfaced in the JSON envelope's `notes` and on stderr.
- Patterns match the path after symlink resolution. When `src/allowed/esc` is a symlink to `../protected`, a Write to `src/allowed/esc/app.ts` is judged as `src/protected/app.ts` and the `src/allowed/**` override does not apply. A resolution that moves the path adds a note, e.g. `override matching followed a symlink: /repo/src/allowed/esc/app.ts resolves to src/protected/app.ts`. A path containing directories that do not exist yet resolves from its nearest existing ancestor.
- Swapping a symlink between the hook's decision and the write itself is out of scope. guardrails runs once, before the write.
