...
The project outcome demands that violation-detection precision and the agent-facing experience improve continuously, yet nothing measures either. There is no corpus of known violations or near-misses, no false-positive tracking, and the `<10ms/file` latency assertion (`assert_under_10ms`) covers 5 of the 43 first-party rule_ids. Rule improvements land on gut feeling, and a precision regression would reach `main` undetected (Issue #255).
...
3. **Toggle granularity is coarser than rule granularity.** `RulesConfig` exposes 29 toggles for 43 rule_ids; the `ast_security` toggle alone gates 13 rule_ids that always run together in production. A "latency per rule_id with only that rule enabled" measurement is unrepresentable for those 13.
...
- **Latency semantics: NFR-001 measures the check pipeline** (parse + rule dispatch via `collect_violations` with all 43 first-party rules on, oxlint off), excluding process spawn and config resolution. The measurement boundary matches the existing `assert_under_10ms`, but the statistic is stricter: `assert_under_10ms` asserts the mean of its iterations, while the harness asserts the median of ≥50 iterations per sample, which tolerates CI runner spikes. A per-rule_id breakdown is impossible for the 13 `ast_security` rule_ids (constraint 3), so per-toggle latency is recorded as diagnostics without an assertion.

### Scope

- The corpus covers exactly the 43 first-party rule_ids. oxlint-delegated rules are excluded: upstream precision is upstream's responsibility, and the harness must not depend on an oxlint binary.
...
The CI precision delta gate reads `overrides` through `scripts/precision_gate.sh`'s `override_report`: it fails when `overrides.leak` or `overrides.overreach` increases base to head (Issue #434), with the same base-predates-the-axis bootstrap skip the rule-level axes use. `toggle_latency_us` remains the only top-level field the gate does not compare.

## Amendment 2026-08-16: gate a per-rule corpus sample count axis; state what the gate does not catch

`scripts/precision_gate.sh`'s `rule_report` gains a second FAIL condition next to the FP-rate cross-multiplication: for each rule_id, `tp + fn + fp + tn` on head must not be smaller than on base (Issue #450). The FP-rate axis alone is blind to a shrunk sample set that happens to preserve the ratio: dropping a TN sample changes `tn` without touching `fp`, so `fp`/`tn` can hold steady or improve while a real check silently disappears from the corpus. A base predating this axis, or lacking a rule_id, still hits the existing SKIP path; there is no separate bootstrap branch to add.

### What this axis catches, and what it does not

| Change to the corpus                                                                                                                                      | Result                                                                                                                                                       |
| --------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| A sample is deleted, so a rule's `tp+fn+fp+tn` drops                                                                                                      | FAIL                                                                                                                                                         |
| A sample is added                                                                                                                                         | No FAIL, growth never trips either axis                                                                                                                      |
| A sample is swapped for an easier or harder one at the same count                                                                                         | No FAIL, the axis only sees the total, and the FP-rate axis only sees the resulting `fp`/`tn`, not which sample produced them                                |
| `harness_config` or `metrics_override_config` (`src/hook/precision.rs`) is relaxed, so the harness exercises fewer rules or fewer/weaker override entries | No FAIL, these functions build the `Config` the corpus runs against, and loosening them changes what a sample proves without changing how many samples exist |
| An override glob pattern is widened to match more paths                                                                                                   | No FAIL unless the corpus already has a Fire sample at a newly matched path, `overrides.leak`/`overreach` above only count paths a fixture exists for        |

The last two rows are out of scope for this gate, not a planned follow-up. Bounding `harness_config`/`metrics_override_config` drift, and pinning how many corpus samples a given override pattern is allowed to match, would each need their own axis and their own definition of worse; Issue #450's backlog notes both without a chosen design.
