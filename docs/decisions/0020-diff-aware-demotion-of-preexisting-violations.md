...
- **Completeness contract (fail-loud).** If the before pass emits any note, the content resolution is degraded, or the before read fails for any reason other than file absence, demotion is cancelled entirely and a note states so. A missing file (new-file Write) means every violation is new; nothing demotes.
- **Origin visibility.** `origin: "preexisting"` marks exactly the demoted violations, the only origin the before-comparison verifies. Survivors carry no origin: a non-allowlisted survivor is never before-compared, so the hook cannot prove it was introduced and makes no claim. Demoted ones render as preexisting in stderr warnings; everything else stays unmarked. The field is omitted via `skip_serializing_if` whenever absent, so toggle-off output and survivor entries keep the wire format byte-identical (ADR-0005 envelope shapes updated accordingly). An "introduced" mark is deferred until a consumer needs it and a verification path for non-allowlisted rules exists.
