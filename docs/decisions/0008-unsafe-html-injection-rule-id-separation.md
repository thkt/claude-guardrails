...
| `src/analysis/ast_security/html.rs` の `check_html_assignment` / `check_document_write` | `SECURITY` (本 ADR 前) | `config.rules.ast_security` | `innerHTML = nonliteral` / `outerHTML = nonliteral` / `document.write[ln](nonliteral)`                                     |
...
> Note: `SECURITY_ISSUES` array は 1 つの array で複数 rule_id (`SECURITY` の 5 entries と `DANGEROUS_INNER_HTML` の 1 entry) を内包する設計。`config.rules.security` toggle で gate する単位として共有しており、`SecurityIssue` struct の `rule_id` field で entry ごとに発火 ID を分離する。`src/rules/test_support.rs` の `REGISTERED_RULE_IDS` でも 2 rule_id 同居が明示されている。
