...
| `hookSpecificOutput` の同居と省略 (e2e)          | `json_を付けた_advisory_の実行では_envelope_と_hook_json_が同居する` / `json_を付けた_blocking_の実行では_hookspecificoutput_が載らない` / `json_を付けた_pass_の実行では_hookspecificoutput_が載らない` / `prefetch_json_emits_success_envelope_when_cached`             |
