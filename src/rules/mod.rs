...
impl Severity {
    /// Maps external linter severity strings to in-house `Severity`. The mapping caps at `High`;
    /// `Critical` is reserved for in-house high-certainty patterns and is never produced from
    /// external input.
